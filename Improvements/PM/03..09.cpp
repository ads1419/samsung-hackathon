...
int main() {
	...
	// string input_str = "with name go to make money create an alram at 8 a.m. tomorrow ";
	ifstream infile("Testing/someSpecialTestCases.txt");
	ofstream outfile("Testing/outputHidden.txt", ios::out);
	string linebuffer;
...
*/
	...
	string input_str = "look for breakfast and rename to lunch in event ";
	cout << give_command(input_str) << "\n";
	map<string, set<string> > :: iterator my;
...
/*	vector<string> :: iterator myitr;
	for(myitr = questions.begin(); myitr != questions.end(); myitr++) {
		...
		// if(!whattosend) 
		outfile << "Case#" << counter++ << ":\n";
		outfile << (give_command(input_str)) << endl;
		map<string, set<string> > :: iterator last;
		for(last = placeholder_values.begin(); last != placeholder_values.end(); ++last) {
				set<string> :: iterator lastfinal;
				for(lastfinal = last->second.begin(); lastfinal != last->second.end(); ++lastfinal)
					outfile << last->first << " : " << "\"" << *lastfinal << "\"\n";
